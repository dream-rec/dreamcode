...
- 新增输出安全过滤：流式响应自动过滤 `<think>/<thinking>` 思考标签内容，避免过程泄露
- 新增 API 请求头兼容：支持在设置页配置额外请求头（JSON），可用于 `x-api-key` 等网关字段
- 优化多渠道适配说明：明确 OpenAI 兼容与 Claude 的 Base URL 使用差异（Claude 自动补 `/v1`）
